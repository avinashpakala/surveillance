...
function FloorMap() {
                    ...
                    {buildings.map((building) => (
                      <div key={building.id} className='building-markers'>
                        {building.cameras.map((camera) => (
                          ...
                          >
                            {camera.name}
                             {camera.healthStatus === "Needs Maintenance" && ( // Check if healthStatus is "Needs Maintenance"
    <MdWarning // Render the warning icon
      style={{
        position: "absolute",
        top: "-15px", // Adjust the position of the icon as needed
        right: "-15px",
        fontSize: "24px",
        color: "red",
  ...
  <button
        ...
        onClick={() => {
          // Code to send alert notification
          alert(`Alert sent for ${camera.name}`);
        }}
      >
        <MdNotifications />
      </button>
